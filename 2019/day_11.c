/*
====================================================================
$File: $
$Date: $
$Revision: $
$Createor: Fuzes Marcell $
$Notice: (C) Copyright 2018 by Fuzes Marcell, All Rights Reserved. $
====================================================================
*/

#include "shared_advent.h"
#include "intcode.h"

s64 IntCode[] = {3,8,1005,8,344,1106,0,11,0,0,0,104,1,104,0,3,8,1002,8,-1,10,1001,10,1,10,4,10,108,1,8,10,4,10,101,0,8,28,1006,0,40,2,1009,2,10,1,1108,13,10,1,1007,6,10,3,8,1002,8,-1,10,101,1,10,10,4,10,1008,8,1,10,4,10,1002,8,1,66,1006,0,91,2,103,5,10,1006,0,12,3,8,102,-1,8,10,1001,10,1,10,4,10,1008,8,0,10,4,10,102,1,8,98,3,8,102,-1,8,10,1001,10,1,10,4,10,1008,8,0,10,4,10,102,1,8,120,1,1001,15,10,2,102,4,10,3,8,1002,8,-1,10,101,1,10,10,4,10,108,0,8,10,4,10,1002,8,1,149,1,106,9,10,1,5,5,10,1,1106,6,10,2,5,15,10,3,8,102,-1,8,10,101,1,10,10,4,10,108,1,8,10,4,10,1001,8,0,187,2,1106,9,10,2,9,13,10,3,8,1002,8,-1,10,101,1,10,10,4,10,1008,8,1,10,4,10,1002,8,1,218,1,1106,3,10,1006,0,13,2,1005,15,10,2,1006,19,10,3,8,102,-1,8,10,101,1,10,10,4,10,108,0,8,10,4,10,1002,8,1,254,2,1108,14,10,1006,0,33,1,7,20,10,2,105,6,10,3,8,1002,8,-1,10,1001,10,1,10,4,10,1008,8,1,10,4,10,101,0,8,292,1006,0,82,2,6,7,10,3,8,102,-1,8,10,101,1,10,10,4,10,108,0,8,10,4,10,1001,8,0,320,1006,0,11,101,1,9,9,1007,9,979,10,1005,10,15,99,109,666,104,0,104,1,21102,932700857100,1,1,21101,0,361,0,1106,0,465,21102,825599210392,1,1,21102,1,372,0,1106,0,465,3,10,104,0,104,1,3,10,104,0,104,0,3,10,104,0,104,1,3,10,104,0,104,1,3,10,104,0,104,0,3,10,104,0,104,1,21101,29195529219,0,1,21101,419,0,0,1106,0,465,21101,0,235324673063,1,21102,1,430,0,1105,1,465,3,10,104,0,104,0,3,10,104,0,104,0,21102,988225098508,1,1,21102,453,1,0,1106,0,465,21102,988753318756,1,1,21101,464,0,0,1106,0,465,99,109,2,22101,0,-1,1,21102,40,1,2,21101,0,496,3,21101,0,486,0,1105,1,529,109,-2,2106,0,0,0,1,0,0,1,109,2,3,10,204,-1,1001,491,492,507,4,0,1001,491,1,491,108,4,491,10,1006,10,523,1101,0,0,491,109,-2,2106,0,0,0,109,4,2102,1,-1,528,1207,-3,0,10,1006,10,546,21102,0,1,-3,21201,-3,0,1,22102,1,-2,2,21101,0,1,3,21102,565,1,0,1105,1,570,109,-4,2105,1,0,109,5,1207,-3,1,10,1006,10,593,2207,-4,-2,10,1006,10,593,22102,1,-4,-4,1105,1,661,22101,0,-4,1,21201,-3,-1,2,21202,-2,2,3,21101,612,0,0,1106,0,570,22101,0,1,-4,21102,1,1,-1,2207,-4,-2,10,1006,10,631,21101,0,0,-1,22202,-2,-1,-2,2107,0,-3,10,1006,10,653,21202,-1,1,1,21101,0,653,0,106,0,528,21202,-2,-1,-2,22201,-4,-2,-4,109,-5,2105,1,0};

typedef enum color_type color_type;
enum color_type
{
    Panel_Black,
    Panel_White,
};

typedef enum turn_type turn_type;
enum turn_type
{
    Turn_Left,
    Turn_Right,
};

enum
{
    Facing_Up,
    Facing_Down,
    Facing_Right,
    Facing_Left,
};

typedef struct tile tile;
struct tile
{
    b32 Painted;
    color_type Color;
};

#define TILE_WIDTH 200
#define TILE_HEIGHT 200
global tile TileMap[TILE_WIDTH * TILE_HEIGHT];

int main()
{
    // TODO(fuzes): The extra dat at the end is probably not needed like this we should fix this later.
    // I first tought this was needed because he will use it as memory but that is probably not true.
    context Context = InitContext(IntCode, sizeof(IntCode), sizeof(s64) * 1024);
    
    int RobotX = 100;
    int RobotY = 100;
    
    TileMap[RobotY * TILE_WIDTH + RobotX].Color = Panel_White;
    
    int RobotDirX = 0;
    int RobotDirY = 1;
    
    int MinX = 100;
    int MinY = 100;
    
    int MaxX = 100;
    int MaxY = 100;
    
    u32 PaintedOnceCounter = 0;
    
    b32 Running = true;
    s64 Input[1] = {Panel_Black};
    while(Running)
    {
        Input[0] = TileMap[RobotY * TILE_WIDTH + RobotX].Color;
        intcode_result Result = RunIntCode(&Context, Input, ArrayCount(Input), 2);
        switch(Result.Event)
        {
            case Event_Type_Halt:
            {
                Running = false;
            } break;
            
            case Event_Type_Output:
            {
                Assert(Result.NumberOfOutputs == 2);
                color_type ColorType = Result.Outputs[0];
                turn_type TurnType = Result.Outputs[1];
                s32 Angle = TurnType == Turn_Left ? 1 : -1;
                s32 Sin = Angle * 1;
                s32 Cos = Angle * 0;
                s32 DirX = RobotDirX;
                s32 DirY = RobotDirY;
                RobotDirX = (Cos * DirX) - (Sin * DirY);
                RobotDirY = (Sin * DirX) + (Cos * DirY);
                
                tile *Tile = TileMap + (RobotY * TILE_WIDTH + RobotX);
                if(!Tile->Painted)
                {
                    Tile->Painted = true;
                    ++PaintedOnceCounter;
                }
                
                Tile->Color = ColorType;
                
                RobotX += RobotDirX * 1;
                RobotY += RobotDirY * 1;
                
                Assert(RobotX < 200);
                Assert(RobotY < 200);
                Assert(RobotX >= 0);
                Assert(RobotY >= 0);
                
                if(RobotX > MaxX)
                {
                    MaxX = RobotX;
                }
                
                if(RobotY > MaxY)
                {
                    MaxY = RobotY;
                }
                
                if(RobotX < MinX)
                {
                    MinX = RobotX;
                }
                
                if(RobotY < MinY)
                {
                    MinY = RobotY;
                }
            }
        }
    }
    
    FILE *File = fopen("registration.bmp", "wb");
    
    bmp_header Header = InitBitmapHeader(TILE_WIDTH, TILE_HEIGHT);
    u32 Bitmap[TILE_HEIGHT * TILE_WIDTH];
    
    for(u32 Index = 0;
        Index < ArrayCount(Bitmap);
        ++Index)
    {
        tile *Tile = TileMap + Index;
        if(Tile->Color == Panel_White)
        {
            Bitmap[Index] = 0xFFFFFFFF;
        }
        else
        {
            Bitmap[Index] = 0x0;
        }
        
    }
    
    fwrite(&Header, sizeof(Header), 1, File);
    fwrite(Bitmap, sizeof(Bitmap), 1, File);
    
    fclose(File);
    
    
    return(0);
}