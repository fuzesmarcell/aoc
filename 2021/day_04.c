...
typedef struct {
	bool32 won;
	bool32 marked[BOARD_HEIGHT][BOARD_WIDTH];
	int32_t numbers[BOARD_HEIGHT][BOARD_WIDTH];
} board_t;

int numbers[] = { 46,79,77,45,57,34,44,13,32,88,86,82,91,97,89,1,48,31,18,10,55,74,24,11,80,78,28,37,47,17,21,61,26,85,99,96,23,70,3,54,5,41,50,63,14,64,42,36,95,52,76,68,29,9,98,35,84,83,71,49,73,58,56,66,92,30,51,20,81,69,65,15,6,16,39,43,67,7,59,40,60,4,90,72,22,0,93,94,38,53,87,27,12,2,25,19,8,62,33,75 };
board_t boards[] = {
    {.numbers = {
    84, 94, 24, 52, 44,
    96, 33, 74, 35, 13,
...
int main(void) {
	...
	for (int i = 0; i < COUNT_OF(numbers); i++) {
		...
		for (int board_idx = 0; board_idx < COUNT_OF(boards); board_idx++) {
			...
			// mark numbers which are hit
			int unmarked_sum = 0;
			for (int y = 0; y < BOARD_HEIGHT; y++) {
				for (int x = 0; x < BOARD_WIDTH; x++) {
					if (n == board->numbers[y][x]) {
						board->marked[y][x] = true;
					}

					if (!board->marked[y][x]) {
						unmarked_sum += board->numbers[y][x];
					}
				}
			}
			...
			for (int y = 0; y < BOARD_HEIGHT; y++) {
				...
				// TODO: Do not check board if it already won in this case!
				if (hit) {
					board->won = true;
					last_winning_board_score = unmarked_sum * n;
					goto next_board;
				}
			}
